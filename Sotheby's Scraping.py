...
''' type in any artist name you want with format e.g."claude monet"/"  Picasso"  '''
#first_name,surname = target[6],target[7] # --- Examples --- #
artist_name = input("Please tell me 'first_name surname' or 'name [blank]':").split()
artist_string = ' '.join(artist_name)
if len(artist_name) == 1:
    first_name = artist_name
elif len(artist_name) == 2:
    first_name, surname = artist_name
...
with open(f"Art index_Sotheby's-{artist_string}.csv", 'a', newline='') as csvfile:
    ...
    json_data = {"requests": [{"indexName": "bsp_dotcom_prod_en",
                               "params": f"highlightPreTag=%3Cais-highlight-0000000000%3E&highlightPostTag=%3C%2Fais-highlight-0000000000%3E&clickAnalytics=true&hitsPerPage=51&filters=type%3A%22Bid%22%20OR%20type%3A%22Buy%20Now%22%20OR%20type%3A%22Lot%22%20OR%20type%3A%22Private%20Sale%22%20OR%20type%3A%22Retail%22&query={artist_string}&maxValuesPerFacet=9999&page=1&facets=%5B%22type%22%2C%22endDate%22%2C%22lowEstimate%22%2C%22highEstimate%22%2C%22artists%22%5D&tagFilters=&facetFilters=%5B%5B%22type%3ALot%22%5D%5D"},
                              {"indexName": "bsp_dotcom_prod_en",
                               "params": f"highlightPreTag=%3Cais-highlight-0000000000%3E&highlightPostTag=%3C%2Fais-highlight-0000000000%3E&clickAnalytics=false&hitsPerPage=1&filters=type%3A%22Bid%22%20OR%20type%3A%22Buy%20Now%22%20OR%20type%3A%22Lot%22%20OR%20type%3A%22Private%20Sale%22%20OR%20type%3A%22Retail%22&query={artist_string}&maxValuesPerFacet=9999&page=1&attributesToRetrieve=%5B%5D&attributesToHighlight=%5B%5D&attributesToSnippet=%5B%5D&tagFilters=&analytics=false&facets=type"}]}
    page = requests.post(
    ...
    for num in range(1,last_page_num+1):
        json_data = {"requests": [{"indexName": "bsp_dotcom_prod_en",
                                "params": f"highlightPreTag=%3Cais-highlight-0000000000%3E&highlightPostTag=%3C%2Fais-highlight-0000000000%3E&clickAnalytics=true&hitsPerPage=51&filters=type%3A%22Bid%22%20OR%20type%3A%22Buy%20Now%22%20OR%20type%3A%22Lot%22%20OR%20type%3A%22Private%20Sale%22%20OR%20type%3A%22Retail%22&query={artist_string}&maxValuesPerFacet=9999&page={num-1}&facets=%5B%22type%22%2C%22endDate%22%2C%22lowEstimate%22%2C%22highEstimate%22%2C%22artists%22%5D&tagFilters=&facetFilters=%5B%5B%22type%3ALot%22%5D%5D"},
                                {"indexName": "bsp_dotcom_prod_en",
                                "params": f"highlightPreTag=%3Cais-highlight-0000000000%3E&highlightPostTag=%3C%2Fais-highlight-0000000000%3E&clickAnalytics=false&hitsPerPage=1&filters=type%3A%22Bid%22%20OR%20type%3A%22Buy%20Now%22%20OR%20type%3A%22Lot%22%20OR%20type%3A%22Private%20Sale%22%20OR%20type%3A%22Retail%22&query={artist_string}&maxValuesPerFacet=9999&page={num-1}&attributesToRetrieve=%5B%5D&attributesToHighlight=%5B%5D&attributesToSnippet=%5B%5D&tagFilters=&analytics=false&facets=type"}]}
        page = requests.post(
            "https://o28sy4q7wu-dsn.algolia.net/1/indexes/*/queries?x-algolia-agent=Algolia for JavaScript (4.2.0); Browser (lite); react (16.13.1); react-instantsearch (6.7.0); JS Helper (3.2.2)&x-algolia-api-key=e732e65c70ebf8b51d4e2f922b536496&x-algolia-application-id=O28SY4Q7WU",
            json=json_data)
    ...
    #'Artistic style',, data['filter']["label_txt"]
        for item in data['results'][0]['hits']:
            #print(target[4], item["artists"][0].split()[0].lower()) ###(first_name or (first_name and surname))
